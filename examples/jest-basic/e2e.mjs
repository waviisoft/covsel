// End-to-end proof of the covsel record -> affected loop against a real Jest
// suite. Copies this example into a throwaway git repo, records the map with the
// jest adapter, then asserts selection under a series of edits. Run with
// `pnpm --filter @covsel/example-jest-basic e2e` after a build.
...
const jestBin = join(exampleDir, 'node_modules', '.bin', 'jest');
...
try {
  for (const entry of ['src', 'test', 'jest.config.mjs', 'tsconfig.json']) {
    cpSync(join(exampleDir, entry), join(tmp, entry), { recursive: true });
  }
  // No "type" field: a stock Jest project is CommonJS, and ts-jest emits the
  // CommonJS this config asks for.
  writeFileSync(join(tmp, 'package.json'), '{\n  "name": "e2e",\n  "private": true\n}\n');
  writeFileSync(join(tmp, '.gitignore'), '.covsel/\nnode_modules/\n');
  // Reuse the example's installed dependencies (jest + ts-jest).
  ...
  console.log('recording map with the jest adapter...');
  const rec = run('node', [covselBin, 'record', '--adapter', 'jest', '--', jestBin], tmp);
  ...
  console.log('scenario: edit the body of src/shared.ts (shared code, block level)');
  writeFileSync(
    join(tmp, 'src', 'shared.ts'),
    readFileSync(join(tmp, 'src', 'shared.ts'), 'utf8').replace(
      '`[covsel] ${s}`',
      '`[covsel!] ${s}`',
    ),
  );
  ...
  console.log('scenario: covsel run -- jest (only affected)');
  writeFileSync(
    join(tmp, 'src', 'a.ts'),
    `${readFileSync(join(tmp, 'src', 'a.ts'), 'utf8')}// edit\n`,
  );
  const ran = run('node', [covselBin, 'run', '--adapter', 'jest', '--', jestBin], tmp);
  assert(ran.status === 0, 'run wraps the runner and exits 0');
  assert(
    /a\.test\.ts/.test(ran.stderr) && !/b\.test\.ts/.test(ran.stderr),
    'the wrapped run executes only a.test.ts',
  );
  git(tmp, ['checkout', '--', 'src/a.ts']);
