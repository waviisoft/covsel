// End-to-end proof of the covsel record -> affected loop against Mocha using the
// generic NODE_V8_COVERAGE path -- zero runner integration, no coverage provider
// and no Mocha-specific package. Copies this example into a throwaway git repo,
// records the map, then asserts selection under a series of edits. Run with
// `pnpm --filter @covsel/example-mocha-basic e2e` after a build.
...
const mochaBin = join(exampleDir, 'node_modules', '.bin', 'mocha');
...
const tmp = mkdtempSync(join(tmpdir(), 'covsel-e2e-mocha-'));
try {
  ...
  // example installs -- reuse the example's own resolved copy, which is also
  // where Mocha itself comes from.
  ...
  const rec = run('node', [covselBin, 'record', '--', mochaBin], tmp);
  ...
  console.log('scenario: covsel run -- mocha (only affected)');
  writeFileSync(
    join(tmp, 'src', 'a.mjs'),
    `${readFileSync(join(tmp, 'src', 'a.mjs'), 'utf8')}// edit\n`,
  );
  const ran = run('node', [covselBin, 'run', '--', mochaBin], tmp);
  assert(ran.status === 0, 'run wraps the runner and exits 0');
  assert(
    /1 passing/.test(`${ran.stdout}${ran.stderr}`),
    'run executes only the affected test file, not the whole suite',
  );
  git(tmp, ['checkout', '--', 'src/a.mjs']);
